...
import { debugCostCalculation } from "./src/utils/debugHelper.js";
...
async function debugPricingDifference() {
  ...
  try {
              ...
              if (entry.timestamp?.startsWith(TARGET_DATE)) {
                ...
                if (debug) {
                  ...
                  if (entriesFound <= 5) {
                    ...
                    console.log(
                      `   Input: ${debug.tokens.input}, Output: ${debug.tokens.output}`,
                    );
                    console.log(
                      `   Cache Creation: ${debug.tokens.cacheCreation}, Cache Read: ${debug.tokens.cacheRead}`,
                    );
                    console.log(`   Cost: $${debug.cost.toFixed(6)}`);
                    console.log(
                      `   Breakdown: Input $${debug.breakdown.inputCost.toFixed(6)}, Output $${debug.breakdown.outputCost.toFixed(6)}, Cache Create $${debug.breakdown.cacheCreationCost.toFixed(6)}, Cache Read $${debug.breakdown.cacheReadCost.toFixed(6)}`,
                    );
                    console.log("");
                  }
                }
    ...
    console.log(
      `  集計データとの差異: $${Math.abs(targetDayUsage.cost - totalDebugCost).toFixed(4)}`,
    );
    console.log("");
    ...
    console.log(
      `  Total: ${(tokenBreakdown.input + tokenBreakdown.output + tokenBreakdown.cacheCreation + tokenBreakdown.cacheRead).toLocaleString()}`,
    );
